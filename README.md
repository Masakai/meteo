...
├── detection_state.py                # 検出ループのグローバル状態 (v3.6.2+)
├── detection_filters.py              # 感度プリセット・鳥フィルタ・薄明パラメータ (v3.6.2+)
├── recording_manager.py              # 手動録画の ffmpeg ジョブ管理 (v3.6.2+)
├── http_handlers.py                  # MJPEGHandler / ThreadedHTTPServer (v3.6.2+)
├── astro_utils.py                    # 検出ウィンドウ（日没〜日出）判定
├── astro_twilight_utils.py           # 薄明期間判定（civil / nautical / astronomical）(v3.5.0+)
├── dashboard.py                      # 複数カメラのダッシュボード
├── dashboard_routes.py               # 検出一覧・削除・統計・設定のルート実装
├── dashboard_camera_handlers.py      # スナップショット・マスク更新・録画・YouTube 配信
├── dashboard_config.py               # バージョン・設定定数
├── dashboard_templates.py            # HTML テンプレート
├── dashboard_templates_settings.py   # 設定画面テンプレート
├── detection_store.py                # SQLite CRUD + JSONL 増分同期 (v3.6.0+)
├── generate_compose.py               # streamersからdocker-compose.yml/go2rtc.yamlを自動生成
├── migrate_camera_dirs.py            # v3.11.0 カメラ名インデックス化マイグレーション
├── convert_detections_mp4_to_mov.py  # 検出クリップのMP4/MOV変換
├── scripts/                          # 運用スクリプト群（SCRIPTS_REFERENCE.md 参照）
...
├── documents/                        # 詳細ドキュメント一式（GitHub Pages 公開）
└── README.md                         # このファイル
```

### 詳細ドキュメント

`documents/` 配下のリファレンスは GitHub Pages でも公開されています。

- [ARCHITECTURE.md](documents/ARCHITECTURE.md) - システム構成・コンポーネント依存・処理フロー
- [API_REFERENCE.md](documents/API_REFERENCE.md) - 全 HTTP エンドポイント仕様
- [CONFIGURATION_GUIDE.md](documents/CONFIGURATION_GUIDE.md) - 環境変数・設定チューニング
- [SETUP_GUIDE.md](documents/SETUP_GUIDE.md) - 初回セットアップ手順
- [OPERATIONS_GUIDE.md](documents/OPERATIONS_GUIDE.md) - 運用手順・バージョン別マイグレーション
- [SECURITY.md](documents/SECURITY.md) - セキュリティ方針・パストラバーサル対策
- [DETECTION_TUNING.md](documents/DETECTION_TUNING.md) - 検出感度チューニング
- [DETECTOR_COMPONENTS.md](documents/DETECTOR_COMPONENTS.md) - 検出エンジン内部モジュール仕様
- [DETECTION_STORE.md](documents/DETECTION_STORE.md) - SQLite 検出結果ストレージ仕様（v3.6.0+）
- [DOCKER_ARCHITECTURE.md](documents/DOCKER_ARCHITECTURE.md) - Docker コンテナ構成
- [ASTRO_UTILS.md](documents/ASTRO_UTILS.md) - 天文計算モジュール
- [SCRIPTS_REFERENCE.md](documents/SCRIPTS_REFERENCE.md) - 運用スクリプトリファレンス
- [TRIANGULATION.md](documents/TRIANGULATION.md) - 多地点三角測量システム
...
単体カメラのWebプレビューでは、マスクの表示/更新、スナップショット取得、再起動要求、ストリーム・検出・マスク状態の確認ができます（`/stats` に実効FPS `runtime_fps` を含む）。

#### オプション

```bash
python meteor_detector_rtsp_web.py \
  rtsp://192.0.2.100:554/stream \
  --output ./detections \
  --sensitivity medium \
  --scale 0.5 \
  --buffer 15 \
...
rtsp://user:pass@192.0.2.11/live
```

`|` 区切りでマスク画像、表示名、YouTube配信キーを指定できます。

```
rtsp://user:pass@192.0.2.25/live | camera1.jpg | 東側
rtsp://user:pass@192.0.2.3/live  | camera2.jpg | 南側
rtsp://user:pass@192.0.2.11/live || 西側 | youtube:xxxx-xxxx-xxxx-xxxx
```
...
rtsp://user:pass@192.0.2.3/live  | camera2.jpg
rtsp://user:pass@192.0.2.11/live
```
...
> バージョン別のマイグレーション手順は [documents/OPERATIONS_GUIDE.md](documents/OPERATIONS_GUIDE.md#バージョン別マイグレーション) に一覧があります。以下は代表的な項目です。

### v3.5.x → v3.6.0（SQLite 移行）

v3.6.0 から検出データの正本は SQLite (`detections.db`) に移行しました。既存の JSONL データを移行するため、アップグレード後に以下を実行してください。
...
DETECTIONS_DIR=./detections python scripts/migrate_jsonl_to_sqlite.py

# 3. コンテナを再起動
./meteor-docker.sh start
```

- 既存の `detections.jsonl` と `detection_labels.json` は削除されません（ロールバック用に保持）
- ロールバックする場合は `detections.db` を削除してコンテナを再起動するだけです
- 詳細は [documents/DETECTION_STORE.md](documents/DETECTION_STORE.md) を参照

### v3.6.2（検出エンジン責務分割）

v3.6.2 で `meteor_detector_rtsp_web.py` からモジュールグローバル状態・フィルタ・録画・HTTP ハンドラが分離され、以下のモジュールに責務分割されました。機能挙動に変更はありません。

- `detection_state.py` / `detection_filters.py` / `recording_manager.py` / `http_handlers.py`

### v3.10.0（マスク自動保護）

ダッシュボードから手動更新したマスクを、次回 `generate_compose.py` 再実行時に上書きしないようになりました。`masks/.generated_hashes.json` に SHA256 ハッシュが記録されます。強制上書きは `python generate_compose.py --force-overwrite-masks`。詳細は [documents/CONFIGURATION_GUIDE.md](documents/CONFIGURATION_GUIDE.md) の「generate_compose.py 再実行時のマスク保護」節を参照。

### v3.10.x → v3.11.0（カメラ名インデックス化）

v3.11.0 で `generate_compose.py` は内部名（`CAMERA{i}_NAME`）を `camera1` / `camera2` / ... に**固定**で書き出すようになりました。旧 IP 含みの内部名（例: `camera1_10_0_1_25`）が残っている場合は、プロジェクトルートの `migrate_camera_dirs.py` で移行します。

!!! warning
    このスクリプトは `--dry-run` を明示しない限り**本実行**します（他の scripts/ 配下と挙動が逆）。最初の実行は必ず `--dry-run` を付けてください。

```bash
# 1. コンテナを停止し、detections/ をバックアップ
./meteor-docker.sh stop
...
詳細は [documents/OPERATIONS_GUIDE.md](documents/OPERATIONS_GUIDE.md#v3110--カメラ名インデックス化アップデート) と [documents/SCRIPTS_REFERENCE.md](documents/SCRIPTS_REFERENCE.md#migrate_camera_dirspy) を参照してください。
...
camera1:
  environment:
    - TZ=Asia/Tokyo
    - RTSP_URL=rtsp://user:pass@192.0.2.25/live  # RFC5737 TEST-NET の例示
    - CAMERA_NAME=camera1                        # v3.11.0+ は camera{i} に固定
    - CAMERA_NAME_DISPLAY=東側                    # ダッシュボード UI の表示名
...
- 2026-04-18: v3.11.0 カメラ名インデックス化（`CAMERA{i}_NAME = camera{i}` 固定）。移行スクリプト `migrate_camera_dirs.py` を追加
- 2026-04-13: v3.10.0 ダッシュボードから手動更新したマスクを `generate_compose.py` 再実行時に保護
- 2026-04-12: v3.6.0 検出データストアを SQLite に移行（パフォーマンス改善・クロスカメラ管理）。v3.6.2 で検出エンジンをモジュール分割
...
- 2026-03-20: v3.5.0 薄明（twilight）動作モード・鳥シルエットフィルタを追加
