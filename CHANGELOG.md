...
- `meteor_detector_realtime.py` — 検出クリップの再生速度異常を、fps推定の廃止によって根本対策した。設計書は `documents/designs/2026-09-17-clip-timestamp-faithful-export.md`。
  ...
  **問題**: v3.17.2・v3.18.0・v3.17.4で3回修正したにもかかわらず、検出クリップが実際の映像より速く再生される事象が再発していた。2026-09-17にgreeng4上の残存クリップ5件について全フレームのOSD時刻をOCRで実測し、実効10fpsの映像が19.75fps・20fpsで書き出されている例（2倍速）と、10.58fps・11.0fpsで正しく書き出されている例が混在することを確認した。

  **原因**: 設計上の欠陥が2点あった。(1) `RingBuffer` はフレームごとの受信時刻を保持しているのに、書き出し時に `estimate_fps_from_frames()` でクリップ全体を1個のfps値へ潰し、ffmpegへ `-r` で渡していた。実効fpsが一定でないカメラ（Tapo C120は夜間IRモードで20fpsから10fps程度へ低下し、フレーム欠落も起きる）では、どの値を選んでもクリップのどこかで再生速度がずれる。(2) その1個の値をフレーム間隔の**中央値**で推定していた。フレームが等間隔で届かない場合、中央値は平均的な間隔を表さない（間隔が0.05秒・0.05秒・0.2秒の繰り返しなら平均は10fpsだが中央値は0.05秒＝20fpsになる）。過去に観測された19.75fps・108fps・210fps・226fpsはこの形で説明できる。過去3回の修正はいずれもこの推定値の後処理であり、推定の入力が偏っている限り正しい値は得られなかった。

  **対策**: fps推定を廃止した。出力fpsはRTSPネゴシエーション値に固定し、新設した `resample_frames_to_cfr()` が各フレームを自身の受信時刻の位置へ再配置する。実効10fpsの区間では同じフレームが2回ずつ入り、欠落区間は直前のフレームで埋まる。これにより再生時間は常に実時間と一致し（誤差は1格子＝1/out_fps秒以内）、クリップ途中でのfps変化にも追従する。再生速度を決める値が推定値でなくなるため、中央値か平均か・クランプ閾値をいくつにするかという調整項目自体が消える。Facebook互換のCFR要件（`Constrained Baseline` / `15360 tbn` / `avc1` / `force-cfr=1`）は維持している。

  副次的な利点として、`out_fps` が常にネゴシエーション値（整数）になったため、`-r f"{target_fps:.0f}"` の丸めで情報が失われなくなり、`ffprobe` の `r_frame_rate` が再びスモークチェックとして使えるようになった（2026-08-24に「19.5〜20.5fpsが全て `20/1` と表示されるため正常判定の根拠にならない」と判明していた問題）。

- **過去の原因説明の訂正**: v3.17.3・v3.17.4のCHANGELOG本文および `estimate_fps_from_frames()` のdocstringは、本事象の原因を「CPU飽和で `cap.read()` の呼び出し間隔が乱れるため」と記述していた。2026-09-17の事例では、発生時刻のホストCPUはidle約60%・ロードアベレージは4コアに対し2〜3であり（`sar` の記録による）、CPUは飽和していなかった。この説明は少なくとも本事例には当てはまらない。過去の事例（2026-08-16の108fps・117fps、2026-08-23の210fps・226fps、2026-08-25の174fps）については、greeng4に当時の `sar` 記録が残っておらず検証できない。「各カメラコンテナ99%前後」という過去の記述は、`docker stats` の1コア基準の値（4コア機では400%が上限）を飽和と解釈した可能性があるが、当時の測定方法は確認できていない。

### Removed
- `meteor_detector_realtime.py` — `estimate_fps_from_frames()` と `max_ratio_to_fallback` によるクランプ処理を削除（上記対策により不要）。`sanitize_fps()` は `RTSPReader` と書き出し関数が引き続き使用するため残置。

### Added
- `meteor_detector_realtime.py` — `resample_frames_to_cfr()` と `ResampleStats`。再配置の統計（出力フレーム数・使用された入力フレーム数・出力に現れなかった入力フレーム数・同一フレームの最大連続回数）を返す。脱落率が `RESAMPLE_DROP_WARN_RATIO`（0.25）を超えた場合に警告ログを出力する。

  閾値は実測に基づく。受信間隔には常にジッタがあり、格子間隔より短い間隔で届いたフレームは同じスロットを奪い合って一方が脱落するため、`n_dropped=0` は正常時の期待値にならない。2026-09-17にgreeng4のcamera1で実測した受信間隔（公称0.05秒に対し0.031〜0.066秒）を再配置にかけると、正常時でも昼7〜11%・夜1.5%が脱落する一方、同時到着バーストでは33%に達する。両者を分離する値として0.25を採った。
- `tests/test_meteor_detector_realtime.py` — 再配置処理のテストを8件追加（10fps→20fps格子での複製、20fps入力での恒等性、中央値方式が2倍速を生む偏った入力、同時到着バースト、クリップ途中のfps変化、欠落区間の補間、退化入力、out_fpsの正規化）。旧 `estimate_fps_from_frames()` のテスト6件を削除。

### Known Issues
- RTSP再接続直後のバースト（OpenCVがバッファ済みフレームを同一時刻で一括に吐き出す状況）では、それらが1〜2個の出力スロットに集約され、大半が `n_dropped` に計上される。再生時間は実時間どおりになるが、その区間の映像内容は失われる。受信時刻が撮影時刻を表していない入力に対する想定内の挙動であり、根本的な解消にはフレーム時刻をストリームのPTS由来に変える必要がある（設計書7章）。
- 検出側の速度（px/s）と `duration` も同じ受信時刻から計算されるため、検出イベント（0.2〜0.6秒）という短い窓の中で受信時刻が局所的に圧縮されていれば、速度が過大に出る可能性がある。camera2/camera3で運用中の `TWILIGHT_MAX_SPEED=700`（薄明期の鳥除外）に影響し、本物の流星を除外しうる。**未検証**であり、本リリースの範囲外（設計書4.5節）。

## [3.17.4] - 2026-08-23
### Fixed
- `meteor_detector_realtime.py` — `estimate_fps_from_frames()` に `max_ratio_to_fallback`（デフォルト1.5）によるクランプ処理を再適用。v3.18.0（コミット `f4c328e`）で導入されたfps推定クランプは、同コミットに含まれていた他2件（同時刻バースト抑制・終了処理の孤児化修正）が検出処理に問題を起こしたため`3.17.3-rollback`（コミット `dddaa45`）でv3.17.3タグ時点まで丸ごと巻き戻された際に、意図せず一緒に失われていた。

  巻き戻し後の本番機greeng4で2026-08-23未明、CPU飽和時にカメラのネゴシエーション値20fpsに対し210fps・226fpsという非物理的な高fps値でMP4が書き出される事象を再確認した（`/output/camera3/meteor_20260823_001142_85bb108f.mp4`・`/output/camera3/meteor_20260823_005927_a6282853.mp4` を実機上でffprobeにより確認）。これは`sanitize_fps()`の上限（120.0）だけでは弾けない範囲の異常値であり、fps推定クランプの再発防止効果が失われていたことが原因。

  ユーザー承認のもと、v3.18.0のうち今回はfps推定クランプのみを再適用する。同時刻バースト抑制・終了処理の孤児化修正の2件は、検出処理への影響が未整理のため本リリースには意図的に含めていない。

### Added
- `tests/test_meteor_detector_realtime.py` — `estimate_fps_from_frames()` のクランプ処理に対するテストを4件追加（ネゴシエーション値を大幅に超える推定fpsの棄却／sanitize範囲内での棄却／比率上限未満の正常な変動の許容／比率上限直下の値の許容）。
...
## [3.15.3] - 2026-05-10
### Fixed
- `dashboard_templates.py` — 夜別検出数グラフの合計値ラベルを透明trace方式からannotations方式（`yref: 'paper'` 固定位置）に変更。データ値に関わらず常にグラフ上部の固定位置に表示されるようになった。
