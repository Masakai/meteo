...
**判定方式は開発中に3回作り直した**。当初の「確定バッチ単位で判定」は、確定（`flush_expired()`）が複数回に分割されると各バッチが少数にしか見えず抑制が効かないケースがあった（`end_time` が実データ同様にばらつく158件バーストで完全に無効化を実測）。次の「到着時カウントの窓判定」は、バースト検知後にカウントをクリアしないとバースト終息直後の孤立イベントを誤って巻き込み、クリアすると継続中のバーストの末尾を取りこぼす、という両端で対になった境界バグがあった。最終的に「到着間隔でクラスタリングする」方式に変更し、この矛盾を構造的に解消した。

本番greeng4の全検出データ16,149件で影響を検証した。本システムは「誤認識を削除し、残ったものが流星」という運用のため、検出動画ファイルが現在も残っているレコード（731件）を「ユーザーが流星と認めたもの」とみなして集計したところ、**731件は1件も破棄されなかった**（`EventMerger` の実クラスに実データをプロセスセッション単位で再生して検証）。破棄対象6,178件はすべて、既にユーザーが誤認識として削除済み（動画ファイルが存在しない）のレコードだった。削除済みレコード15,418件の40.07%にあたり、本機能があればこれらは最初から記録されず削除の手間も発生しなかったことになる。抑制の契機となった2件のバースト（camera2の158件、camera1の187件）はいずれも実クラスの再生で100%破棄されることを確認済み。
...
**既知の限界**: 本修正は非物理的なfps値の記録を防ぐが、正しいfpsを復元するものではない。当該2件についてOSD（カメラが映像に焼き込むタイムスタンプ）で実測した真の実効fpsは 10.8fps（East, 65フレーム/6秒）と 5.6fps（South, 96フレーム/17秒）であり、クランプ後の 20fps はこれを上回る。結果として修正後も約1.9倍速・約3.6倍速での再生が残る。フレームに付与される受信時刻そのものが圧縮されているため、その時刻から算出する方式（中央値・全体平均のいずれも）では原理的に復元できない。真の実効fpsを得るにはカメラ側のPTSを使うなど別方式が必要で、本バージョンでは対応していない。
- `meteor_detector_rtsp_web.py` — 検出ループのイベント保存部で `stop_flag` を確認するようにした。従来は `stop_flag` のチェックがループ先頭のフレーム取得部にしかなく、1周の内側で多数のイベントが確定すると `save_meteor_event()` のMP4書き出しが件数分だけ直列に走り、停止要求が来てもループを抜けられなかった。メインスレッドの `detection_thread.join(timeout=5.0)` がタイムアウトして後始末が先に進むため、「終了 - 検出数: N個」ログを出力した後も検出スレッドが生き残って保存を続ける状態になっていた（2026-08-15 01:55 の camera2 のログに、検出#9の直後に「終了 - 検出数: 9個」が出力された後も #10 以降の保存が継続した実例がある）。ユーザーがWebUIで検出結果を削除しても同じ時刻のレコードが次々に再作成される症状の原因。あわせて `join()` がタイムアウトした場合に警告を出すようにした。

**この対策は完全な解消ではなく緩和である**。検出ループを抜けた後の終了処理（`merger.flush_all()` の残りイベントを保存するループ）には `stop_flag` によるチェックを入れていない。停止要求が来ること自体が終了処理に到達する契機のため、ここで `stop_flag` により打ち切ると常に保存0件になってしまう。代わりに `SHUTDOWN_SAVE_BUDGET_SEC`（既定 `4.0` 秒）の時間予算を設け、超過したら残りの保存を打ち切りWARNログを出す方式にした。予算チェックは各保存呼び出しの**前**にあるため、実際の超過幅は予算に加えて直近1件の `save_meteor_event()` の処理時間ぶん上乗せされうる。同時刻バースト抑制（後述）により `flush_all()` が返す残件数自体が大幅に減るため、通常運用でこの経路が問題になることは想定していないが、根本的な解消ではない点に留意すること。

2026-08-16に本番greeng4で、接続時20.0fpsのカメラに対し検出動画が108fps・117fpsで書き出される事象が発生した（`camera1/meteor_20260816_032802_79d9e49b.mp4`、`camera2/meteor_20260816_030601_7f6b0f81.mp4`）。CPU飽和で `cap.read()` の呼び出し間隔が乱れると、フレームに付与される受信時刻（`time.time()` ベース）が実際の撮影間隔より詰まり、`estimate_fps_from_frames()` の推定値がカメラの実効fpsを大きく上回る。これらの値は `sanitize_fps()` の上限120.0の内側にあるため既存の上限チェックでは弾けなかった。カメラのネゴシエーション値を基準とする比率判定を加えることで、夜間IRモードでの実効fps低下（20fps→10fps程度）は従来どおり尊重しつつ、非物理的な高fpsのみを除外する。

なお本件は v3.17.3 で対策した経路（ウォッチドッグの誤検知による検出プロセス強制再起動 → 内部バッファのフレーム一括流入）とは**別原因**である。当該2件が発生した時刻の前後で、対象カメラにプロセス再起動・再接続・ストリーム断はログ上発生していない。また v3.17.2 で修正した「常時30fps固定バグ」とも別で、書き出し側は渡されたfpsを忠実に記録しており、破綻していたのは書き出し前段の実効fps推定値そのものだった。
