...
ダッシュボードは `/image/`, `/detection/`, `/manual_recording/` 等のエンドポイントでリクエストパスを受け取ります。現行実装では以下の多層防御を適用しています。

- **許可リストによるサフィックス検証**: 手動録画削除 (`DELETE /manual_recording/`) はリクエストパスの拡張子として `.mp4` のみを許可しています。削除実行時は同名の `.jpg` サムネイルも自動削除されますが、これはサーバー側で制御された処理であり外部からパスを指定することはできません。go2rtcアセット (`/go2rtc_asset/`) は `video-stream.js` / `video-rtc.js` のみを許可しています。
- **`manual_recordings` ディレクトリ要件**: 手動録画パスには `manual_recordings` パート含有を必須チェックしています。
...
- [ ] 不要なポートを閉じる（go2rtc の 1984 番ポートは WebRTC を使用する場合のみ公開）
- [ ] go2rtc ポート (1984) を外部公開する場合は信頼できるネットワークに限定
- [ ] ファイアウォール設定を確認
- [ ] Dockerイメージの脆弱性スキャン
