...
### ダッシュボードからのカメラ設定（一括／個別）（/settings）

ダッシュボード左サイドバーの「設定」から、全カメラへ一括、または特定 1 カメラのみへ個別に設定を反映できます。

ツールバーの「対象カメラ」ドロップダウンで適用先を選びます（v3.16.0+）。

- **全カメラ**（既定）: `全カメラに適用` ボタンが `POST /camera_settings/apply_all` を実行し、全カメラへ同じ設定を反映。
- **特定カメラ**（例: `camera1`）: `<カメラ名> に適用` ボタンが `POST /camera_settings/apply_one` を実行し、そのカメラの `runtime_settings/<camera>.json` のみ更新。他カメラは変化しません。

ボタン操作:
- `現在値を読み込む`: 選択中の対象カメラの現在設定をフォームへ反映（対象を切り替えただけでは自動反映しません。未保存の編集を誤って上書きしないため、明示操作です）。
...
#### カメラ交換時の個別調整手順（v3.16.0+）

カメラを交換したり、設置環境の差（光害・画角・ノイズ源）で 1 台だけ調整したい場合の手順です。

1. `/settings` を開く。
2. 「対象カメラ」ドロップダウンで調整したいカメラを選ぶ。
3. `現在値を読み込む` を押し、そのカメラの現在設定をフォームに反映する。
4. パラメータを調整する（差分はハイライト表示される）。
5. `<カメラ名> に適用` を押す。そのカメラの `runtime_settings/<camera>.json` のみが更新され、他カメラの設定は変わらない。
6. 即時反映項目はその場で、`sensitivity`/`scale`/`buffer`/`extract_clips` は当該カメラの自動再起動で反映される。
