...
### ダッシュボード設定（一括／個別）（再ビルド不要） v1.13.0+ / 個別はv3.16.0+

`/settings` から全カメラへ一括、または特定 1 カメラのみへ個別に設定を反映できます。

- `GET /settings`: 設定UI
- `GET /camera_settings/current`: 現在値取得（全カメラ分を `results[]` で返す）
- `POST /camera_settings/apply_all`: 全カメラ一括反映
- `POST /camera_settings/apply_one`: 指定 1 カメラのみへ反映（v3.16.0+）

#### 対象カメラの選択（v3.16.0+）

`/settings` ツールバーの「対象カメラ」ドロップダウンで適用先を切り替えます。

- **全カメラ**（既定）: 従来どおり全カメラへ一括適用（`apply_all`）。
- **特定カメラ**（例: `camera1`）: そのカメラの `runtime_settings/<camera>.json` のみを更新し、他カメラは変化しません（`apply_one`）。

カメラ交換や設置環境の差（光害・画角・ノイズ源）で 1 台だけ調整したい場合に使います。対象カメラを切り替えてもフォームは自動で書き換わりません。「現在値を読み込む」ボタンを押すと、選択中のカメラの現在設定がフォームへ反映されます（未保存の編集を誤って上書きしないため明示操作）。
...
2. 「対象カメラ」ドロップダウンで適用先を選択（全カメラ または 特定カメラ）（v3.16.0+）
3. 変更したいパラメータを入力（差分のみハイライト表示）。特定カメラの現在値を基準にしたい場合は「現在値を読み込む」を先に押す
4. 適用ボタンで反映（全カメラ選択時は「全カメラに適用」、個別選択時は「<カメラ名> に適用」）
5. 即時反映項目はリアルタイムで適用、再起動項目は自動で再起動
6. `output/runtime_settings/<camera>.json` に永続化され、次回起動時も有効
